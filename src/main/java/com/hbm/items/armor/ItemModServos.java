...
public class ItemModServos extends ItemArmorMod {
	...
	public void addDesc(List list, ItemStack stack, ItemStack armor) {
		...
		if(item.armorType == 1) {
			if(this == ModItems.servo_set) {
				list.add(EnumChatFormatting.DARK_PURPLE + "  " + stack.getDisplayName() + " " +
					I18nUtil.resolveKey("item.servo_set.chestplate"));
			}
			if(this == ModItems.servo_set_desh) {
				list.add(EnumChatFormatting.DARK_PURPLE + "  " + stack.getDisplayName() + " " +
					I18nUtil.resolveKey("item.servo_set_desh.chestplate"));
			}
		}
		...
		if(item.armorType == 2) {
			if(this == ModItems.servo_set) {
				list.add(EnumChatFormatting.DARK_PURPLE + "  " + stack.getDisplayName() + " " +
					I18nUtil.resolveKey("item.servo_set.leggings"));
			}
			if(this == ModItems.servo_set_desh) {
				list.add(EnumChatFormatting.DARK_PURPLE + "  " + stack.getDisplayName() + " " +
					I18nUtil.resolveKey("item.servo_set_desh.leggings"));
			}
		}
	}
