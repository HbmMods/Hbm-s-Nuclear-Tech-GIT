...
public class GeneralConfig {
	...
	public static void loadFromConfig(Configuration config) {
		...
		magazineMode = enable528 ? MagazineMode.HIDEOUS : MagazineMode.values()[MathHelper.clamp_int(CommonConfig.createConfigInt(config, CATEGORY_WEAPONS, "17.01_gunMagazineMode", "Enables guns to use magazines (or proper belts), in the given modes:\n\t0 = OFF - behavior as it was before, recommended if you're BORING\n\t1 = FAST - fill magazines quickly and automatically, belts may be considered connected, the recommended choice for normal people\n\t2 = IMMERSIVE - fill magazines more manually, empty magazines can be loaded by accident, recommended for people that like a little more realism\n\t3 = HIDEOUS - fill and unload magazines one round at a time, some magazines may only accept one or a few types of ammo, but not tell you; Belts are not interconnected; Automatically enabled with 528 mode; Recommended if you like pain and/or hate yourself", 0), 0, 3)];
	}
}
