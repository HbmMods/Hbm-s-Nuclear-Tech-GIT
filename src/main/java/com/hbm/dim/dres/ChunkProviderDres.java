...
//if theres a way where this shit can be polymorphized so i dont have 2002103012031203210301203120312030123012030120321 variables not being called for shit then PUH-LEASE
	...
	public ChunkProviderDres(World DresWorld, long il, boolean ib) {
		...
		// DimensionManager.getInstance().getDimensionProperties(p_i2006_1_.provider.dimensionId);
		...
		// float atmDensity =
		// ((WorldProviderPlanet)worldObj.provider).getAtmosphereDensity(0,0);
	...
	public BlockMetacoupling getChunkPrimer(int p_73154_1_, int p_73154_2_) {
		...
		// this.ravineGenerator.func_151539_a(this, this.worldObj, p_73154_1_,
		// p_73154_2_, ablock.ablock);

		// if(this.craterGenerator != null)
		// this.craterGenerator.func_151539_a(this, this.worldObj, p_73154_1_,
		// p_73154_2_, ablock.ablock);
	...
	public void populate(IChunkProvider p_73153_1_, int p_73153_2_, int p_73153_3_) {
		 ...
		 * ((WorldProviderMoon)worldObj.provider).getDimensionProperties(k,
		 * l).hasRivers();
		 * 
		 * if(populationFlag )
		 * MinecraftForge.EVENT_BUS.post(new PopulateChunkEvent.Pre(p_73153_1_,
		 * worldObj, rand, p_73153_2_, p_73153_3_, flag));
		 * 
		 * if (this.mapFeaturesEnabled)
		 * {
		 * //TODO: maybe features
		...
		/*
		 * if (TerrainGen.populate(p_73153_1_, worldObj, rand, p_73153_2_, p_73153_3_,
		 * flag, ANIMALS))
		 * {
		 ...
		 * worldObj.provider.getClass() == WorldProviderPlanet.class) {
		 * 
		 * if(DimensionManager.getInstance().getDimensionProperties(worldObj.provider.
		 * dimensionId).isTerraformed()) {
		 * Chunk chunk = worldObj.getChunkFromChunkCoords(p_73153_2_, p_73153_3_);
		 * PlanetEventHandler.modifyChunk(worldObj,
		 * (WorldProviderPlanet)worldObj.provider, chunk);
		 * }
		 * }
		 * 
		 * if (populationFlag)
		 * MinecraftForge.EVENT_BUS.post(new PopulateChunkEvent.Post(p_73153_1_,
		 * worldObj, rand, p_73153_2_, p_73153_3_, flag));
		 * 
		 * OreGenProperties oreGenProperties =
		 * DimensionManager.getInstance().getDimensionProperties(this.worldObj.provider.
		 * dimensionId).getOreGenProperties(this.worldObj);
		 * 
		 * if(oreGenProperties != null) {
		 * for(OreEntry entry : oreGenProperties.getOreEntries()) {
		 * new CustomizableOreGen(entry).generate(rand, p_73153_2_, p_73153_3_,
		 * this.worldObj, this, this.worldObj.getChunkProvider());
		 * }
		 * }
		 */
