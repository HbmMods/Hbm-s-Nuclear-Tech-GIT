...
public class EntityScutterfish extends EntityWaterMob implements IAnimals {
	...
	protected void updateAITasks() {
		...
		if(!this.isInWater()) {
		       this.rotationPitch += (float)0.2;
		        ...
		        if (this.onGround) {
		            this.addVelocity(0.1 * EntityScutterfish.rand.nextDouble() - 0.1 * EntityScutterfish.rand.nextDouble(), 0.0, 0.1 * EntityScutterfish.rand.nextDouble() - 0.1 * EntityScutterfish.rand.nextDouble());
		        }
		}
	}
    ...
    private void updateSwimTarget() {
        if (this.rand.nextInt(100) == 0) {
            double targetX = this.posX + (this.rand.nextDouble() - 0.5) * 10.0;
            double targetY = MathHelper.clamp_double(this.posY + (this.rand.nextDouble() - 0.5) * 4.0, 0, this.worldObj.getHeight() - 1);
            double targetZ = this.posZ + (this.rand.nextDouble() - 0.5) * 10.0;
    ...
    private void moveTowardsTarget() {
        ...
        this.motionX += dirX * 0.1;
        this.motionY += dirY * 0.1;
        this.motionZ += dirZ * 0.1;
    }
