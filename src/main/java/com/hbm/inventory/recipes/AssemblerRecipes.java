...
public class AssemblerRecipes {
	...
	private static void generateList() {
		...
		for(CraftingStack a : HBMRecipes.removeFromFolder)
			hidden.put((ComparableStack) a.aStack(),new HashSet() {{ add(ModItems.assembly_template); }});
		...
		if(stack != null && stack.getItem() instanceof ItemAssemblyTemplate) {
			...
			if(i >= 0 && i < recipeList.size()) {
				...
				if(out != null) {
					...
					if(ret == null)
						ret = recipes.get(new RecipesCommon.ComparableNBTStack(out));
...
//		hidden.put(new ComparableStack(ModItems.sat_gerald, 1), new HashSet() {{ add(ModItems.journal_bj); }});
		...
		for(Map.Entry<Pair<ItemStack,Integer>,ArrayList<CraftingStack>> entry : HBMRecipes.assemblerRecipes.entrySet()) {
			ArrayList<AStack> cStacks = new ArrayList<>();
			for(CraftingStack a : entry.getValue())
					cStacks.add(a.aStack());
			AStack[] aStacks = new AStack[cStacks.size()];
			if(entry.getKey().first().hasTagCompound()) {
				makeRecipe(new RecipesCommon.ComparableNBTStack(entry.getKey().first()).addNBT(entry.getKey().first().getTagCompound()), cStacks.toArray(aStacks), entry.getKey().second());
			} else
				makeRecipe(new ComparableStack(entry.getKey().first()), cStacks.toArray(aStacks), entry.getKey().second());
		}
