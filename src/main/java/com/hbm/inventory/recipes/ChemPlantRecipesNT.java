...
public class ChemPlantRecipesNT
{
	...
	public static void registerChemPlantRecipes()
	{
		...
		recipes.put("LF_BASE", new ChemPlantRecipe(400)
				...
				.setItemInput(new OreDictStack("dustAluminum", 2), new OreDictStack("dustBromine"), new OreDictStack("dustLithium", 3), new OreDictStack("dustFluorite", 3))
		...
		recipes.put("LF_U", new ChemPlantRecipe(200)
				...
				.setItemInput(new OreDictStack("dustU235"), new OreDictStack("nuggetBeryllium", 3))
		...
		recipes.put("LF_SA", new ChemPlantRecipe(300)
				...
				.setItemInput(new ComparableStack(ModItems.powder_nitan_mix, 2), new ComparableStack(ModItems.billet_beryllium))
		...
		recipes.put("ALCOHOL", new ChemPlantRecipe(200)
				...
				.setItemInput(new OreDictStack("dustSulfur", 2))
		...
		recipes.put("SARIN", new ChemPlantRecipe(1200)
				...
				.setItemInput(new OreDictStack("dustRedPhosphorus", 4), new OreDictStack("dustFluorite", 4))
				.setFluidOutput(new FluidStack(1000, FluidType.SARIN), null));
		// Stop-gap for a stop-gep eh?
		for (EnumChemistryTemplate chem : EnumChemistryTemplate.values())
		{
			...
			else
			{
				...
				AStack[] iIn = new AStack[4];
				List<ItemStack> toIn = MachineRecipes.getChemInputFromTempate(pseudoStack);
				if (toIn != null && !toIn.isEmpty())
					iIn = RecipesCommon.itemStackToAStack(toIn.toArray(new ItemStack[toIn.size()]), false);
	...
	{
		public static final ChemPlantRecipe NONE = new ChemPlantRecipe().setFluidInput(new FluidStack(0, FluidType.NONE), new FluidStack(0, FluidType.NONE)).setItemInput(Library.filledArray(AStack.class, new ComparableStack(ModItems.nothing), 4)).setFluidOutput(new FluidStack(0, FluidType.NONE), new FluidStack(0, FluidType.NONE)).setItemOutput(Library.filledArray(ItemStack.class, new ItemStack(ModItems.nothing), 4));
		public int time = 100;
