...
public class AnnihilatorRecipes extends SerializableRecipe {
	...
	public void readRecipe(JsonElement recipe) {
		...
		boolean is528 = obj.get("is528").getAsBoolean();
		if(!(!GeneralConfig.enable528 && is528)) {
			...
			for (JsonElement e : milestones) {
				...
				milestoneList.add(new Pair(milestone.get("amount").getAsBigInteger(), this.readBlueprintItemStack(milestone.get("payout").getAsJsonArray())));
			}
	...
	public void writeRecipe(Object recipe, JsonWriter writer) throws IOException {
		...
		for(Pair<BigInteger, ItemStack> milestone : rec.getValue().milestones) {
			...
			writer.name("payout");
			this.writeBlueprintItemStack(milestone.getValue(), writer);
			writer.endObject();
		}
		writer.endArray();
	}
