...
public class SuperComputerRecipes extends GenericRecipes<GenericRecipe> {
	...
	public void registerDefaults() {
		...
		// process
		registerTriplet("com.processflight", 30 * min, 15 * min, 5 * min, EnumDriveType.DISK_FLIGHTDATA, EnumDriveType.DISK_FLIGHTDATA_PROCESSED, 99, 95, 90);
	...
	protected void registerSimulation(EnumDriveType type, String name) {
		...
		int min = 60 * 20;
		registerTriplet(name, 15 * min, 5 * min / 2, 1 * min, EnumDriveType.FLASH_EMPTY, type, 95, 50, 25);
	}
	
	protected void registerTriplet(String name, int time0, int time1, int time2, EnumDriveType input, EnumDriveType output, int chance0, int chance1, int chance2) {
		...
		this.register(new GenericRecipe(name + "_water").setup(time0, 10_000)
				...
				.outputItems(new ChanceOutputMulti(
						...
						new ChanceOutput(new ItemStack(ModItems.drive, 1, EnumDriveType.FLASH_BROKEN.ordinal()), 100 - chance0)))
		...
		this.register(new GenericRecipe(name + "_pfm").setup(time1, 10_000)
				...
				.outputItems(new ChanceOutputMulti(
						...
						new ChanceOutput(new ItemStack(ModItems.drive, 1, EnumDriveType.FLASH_BROKEN.ordinal()), 100 - chance1)))
		...
		this.register(new GenericRecipe(name + "_helium").setup(time2, 10_000)
				...
				.outputItems(new ChanceOutputMulti(
						...
						new ChanceOutput(new ItemStack(ModItems.drive, 1, EnumDriveType.FLASH_BROKEN.ordinal()), 100 - chance2)))
