...
public class TileEntityElectrolyser extends TileEntityMachineBase implements IEnergyReceiverMK2, IFluidStandardTransceiver, IControlReceiver, IGUIProvider, IUpgradeInfoProvider {
	...
	public void updateEntity() {
		...
		if(!worldObj.isRemote) {
			...
			usageOre = (usageOreBase - usageOreBase * powerLevel / 4) * (amps + 1);
			usageFluid = (usageFluidBase - usageFluidBase * powerLevel / 4) * (amps + 1);
			...
			if(this.leftStack != null) {
				...
				MaterialStack didPour = CrucibleUtil.pourFullStack(worldObj, xCoord + 0.5D + dir.offsetX * 5.875D, yCoord + 2D, zCoord + 0.5D + dir.offsetZ * 5.875D, 6, true, toCast, MaterialShapes.INGOT.q(1), impact);
