...
import com.hbm.inventory.container.ContainerVVER;
...
import com.hbm.inventory.gui.GUIVVER;
...
public class TileEntityVVERController extends TileEntityMachineBase implements IGUIProvider, IControlReceiver, SimpleComponent, IFluidStandardTransceiver, CompatHandler.OCComponent {
	...
	public TileEntityVVERController() {
		super(3);
	...
	public void setup(HashMap<BlockPos, Block> partMap, HashMap<BlockPos, Block> rodMap) {
		...
		for(Entry<BlockPos, Block> entry : partMap.entrySet()) {
			...
			if(block == ModBlocks.vver_fuel) rodCount++;
			if(block == ModBlocks.vver_heatex) heatexCount++;
			if(block == ModBlocks.vver_channel) channelCount++;
			if(block == ModBlocks.vver_heatsink) heatsinkCount++;
			if(block == ModBlocks.vver_neutron_source) sourceCount++;
			if(block == ModBlocks.vver_port) ports.add(entry.getKey());
		}
		...
		for(Entry<BlockPos, Block> entry : rodMap.entrySet()) {
			...
			for(ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
				...
				for(int i = 1; i < 16; i++) {
					...
					if(atPos == null || atPos == ModBlocks.vver_casing) break;
					if(atPos == ModBlocks.vver_control) controlled = true;
					if(atPos == ModBlocks.vver_fuel) {
						if(controlled) {
					...
					if(atPos == ModBlocks.vver_reflector) {
						if(controlled) {
	...
	public String getName() {
		return "container.vverController";
	}
		...
		if(!worldObj.isRemote) {
			...
			if(this.assembled) {
				...
				if(this.unloadDelay <= 0) {
					...
					if((typeLoaded == -1 || amountLoaded <= 0) && slots[0] != null && slots[0].getItem() == ModItems.vver_fuel) {
						typeLoaded = slots[0].getItemDamage();
						amountLoaded++;
						this.decrStackSize(0, 1);
						this.markChanged();
					} else if(slots[0] != null && slots[0].getItem() == ModItems.vver_fuel && slots[0].getItemDamage() == typeLoaded && amountLoaded < rodCount){
						amountLoaded++;
						this.decrStackSize(0, 1);
						this.markChanged();
					}
					...
					if(typeLoaded != -1 && amountLoaded > 0) {
						...
						EnumVVERFuel fuel = EnumUtil.grabEnumSafely(EnumVVERFuel.class, typeLoaded);
						...
						if(this.progress >= this.processTime) {
							...
							if(slots[1] == null) {
								slots[1] = new ItemStack(ModItems.vver_fuel_hot, 1, typeLoaded);
							} else if(slots[1].getItem() == ModItems.vver_fuel_hot && slots[1].getItemDamage() == typeLoaded && slots[1].stackSize < slots[1].getMaxStackSize()) {
								slots[1].stackSize++;
							}
	...
	public boolean isItemValidForSlot(int slot, ItemStack stack) {
		if(slot == 0) return stack.getItem() == ModItems.vver_fuel;
		return false;
	}
	...
	public Container provideContainer(int ID, EntityPlayer player, World world, int x, int y, int z) {
		return new ContainerVVER(player.inventory, this);
	}

	@Override
	@SideOnly(Side.CLIENT)
	public Object provideGUI(int ID, EntityPlayer player, World world, int x, int y, int z) {
		return new GUIVVER(player.inventory, this);
	}
