...
public class TileEntityMachineChemicalFactory extends TileEntityMachineBase implements IEnergyReceiverMK2, IFluidStandardTransceiverMK2, IUpgradeInfoProvider, IControlReceiver, IGUIProvider, IProxyDelegateProvider, IConditionalInvAccess, IRORValueProvider, IRORInteractive {
	...
	public String runRORFunction(String name, String[] params) {
		for(int i = 0; i < 4; i++) {
			if((PREFIX_FUNCTION + "setrecipe" + (i + 1)).equals(name) && params.length == 1) {
				this.chemplantModule[i].setRecipe(params[0], true);
				this.markChanged();
				return null;
			}
		}
