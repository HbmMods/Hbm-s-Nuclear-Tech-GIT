...
public class TileEntityTowerChimney extends TileEntityCondenser { //actually bad holy shit. i dont want to work on this anymore.
	
	//this is meant for a modeling test, things might improve in the future.
	
	public TileEntityTowerChimney() {
		tanks = new FluidTank[2];
		tanks[0] = new FluidTank(Fluids.CARBONDIOXIDE, 1000, 0);
	...
	public void updateEntity() {
		...
		if(worldObj.isRemote) {
			...
			if(this.waterTimer > 0 && this.worldObj.getTotalWorldTime() % 2 == 0) {
				...
				data.setFloat("lift", 2F);
				data.setFloat("base", 0.5F);
				data.setFloat("max", 4F);
				data.setInteger("life", 450 + worldObj.rand.nextInt(250));
