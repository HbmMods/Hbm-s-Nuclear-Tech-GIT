...
public class SatelliteRadar extends SatelliteBase {
	...
	public static final String CMD_GETPLAYERNAME = "getplayername";
	...
	public void onCommandImpl(World world, String... cmd) {
		...
		if(cmd[0].equals(CMD_GETPLAYERNAME) && cmd.length == 2) {
			Entity target = getTargetFromIndex(cmd[1]);
			if(target == null) { this.tx = ""; return; }
			if (target instanceof net.minecraft.entity.player.EntityPlayer) {
				net.minecraft.entity.player.EntityPlayer p = (net.minecraft.entity.player.EntityPlayer) target;
				this.tx = p.getCommandSenderName();
			}
			else{
				this.tx = "Entry not a player";
			}
			return;
		}
	}
