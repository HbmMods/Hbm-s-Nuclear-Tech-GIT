			...
			for(int i = 0; i < 4; i++) {
				for(int j = 0; j < 4; j++) {
					...
					if(world.rand.nextBoolean() || memes) {
						...
						ItemStack nuke = memes ? ModItems.ammo_nuke.stackFromEnum(AmmoFatman.PUMPKIN) : type == 0 ? ModItems.ammo_nuke.stackFromEnum(AmmoFatman.STOCK) : type <= 5 ? ModItems.ammo_nuke.stackFromEnum(AmmoFatman.LOW) : ModItems.ammo_nuke.stackFromEnum(AmmoFatman.SAFE);
						loot.addItem(nuke, -0.375 + i * 0.25, 0, -0.375 + j * 0.25);
					}
				}
			}
		}
