...
import com.hbm.tileentity.machine.pile.TileEntityPileThorium;
...
public class BlockGraphiteThorium extends BlockGraphiteDrilledTE implements IToolable {

	@Override
	public TileEntity createNewTileEntity(World world, int meta) {
		return new TileEntityPileThorium();
	}
	...
	public void registerBlockIcons(IIconRegister iconRegister) {
		...
		this.blockIconAluminum = iconRegister.registerIcon(RefStrings.MODID + ":block_graphite_thorium_aluminum");
	}
		...
		if(!world.isRemote) {
			...
			if(tool == ToolType.SCREWDRIVER) {
				...
				if(side == cfg * 2 || side == cfg * 2 + 1) {
					...
					this.ejectItem(world, x, y, z, ForgeDirection.getOrientation(side), new ItemStack(ModItems.pile_rod_thorium));
				}
			}
			
			if(tool == ToolType.HAND_DRILL) {
				TileEntityPileThorium pile = (TileEntityPileThorium) world.getTileEntity(x, y, z);
	...
	protected Item getInsertedItem() {
		return ModItems.pile_rod_thorium;
	}
}