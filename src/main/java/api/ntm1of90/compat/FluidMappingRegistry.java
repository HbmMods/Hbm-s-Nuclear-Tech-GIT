...
public class FluidMappingRegistry {
    ...
    public static void initialize() {
        ...
        for (String forgeName : forgeToHbmMap.keySet()) {
            if (FluidRegistry.getFluid(forgeName) == null) {
    ...
    private static void addFluidAliases(FluidType fluidType) {
        ...
        if (lowercaseName.contains("_")) {
            ...
            for (String forgeName : FluidRegistry.getRegisteredFluids().keySet()) {
    ...
    public static Fluid getForgeFluid(FluidType type) {
        ...
        Fluid fluid = FluidRegistry.getFluid(hbmName);
        if (fluid != null) {
        ...
        for (String forgeName : FluidRegistry.getRegisteredFluids().keySet()) {
            ...
            if (cleanHbm.equals(cleanForge)) {
                ...
                return FluidRegistry.getFluid(forgeName);
            }
        }
        ...
        if (type == Fluids.PETROIL) {
            Fluid fuelFluid = FluidRegistry.getFluid("fuel");
            if (fuelFluid != null) {
                registerFluidMapping("fuel", type);
                return fuelFluid;
            }
        } else if (type == Fluids.BIOFUEL) {
            Fluid ethanolFluid = FluidRegistry.getFluid("ethanol");
            if (ethanolFluid != null) {
                registerFluidMapping("ethanol", type);
                return ethanolFluid;
            }
        } else if (type == Fluids.OIL) {
            Fluid crudeOilFluid = FluidRegistry.getFluid("crude_oil");
        ...
        if (FluidRegistry.getFluid(hbmName) == null) {
