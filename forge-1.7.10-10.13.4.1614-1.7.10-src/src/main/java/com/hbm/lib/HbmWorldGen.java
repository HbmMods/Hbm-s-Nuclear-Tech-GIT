...
public class HbmWorldGen implements IWorldGenerator {
	...
	private void generateSurface(World world, Random rand, int i, int j) {
		...
		for (int k = 0; k < WorldConfig.cobaltSpawn; k++) {
			int randPosX = i + rand.nextInt(16);
			int randPosY = rand.nextInt(25);
			int randPosZ = j + rand.nextInt(16);
			
			(new WorldGenMinable(ModBlocks.ore_cobalt, 4)).generate(world, rand, randPosX, randPosY, randPosZ);
		}
	...
	private void generateNether(World world, Random rand, int i, int j) {
		...
		for (int k = 0; k < 10; k++)
		{
			...
			(new WorldGenMinable(ModBlocks.ore_nether_cobalt, 10, Blocks.netherrack)).generate(world, rand, randPosX, randPosY, randPosZ);
		}
